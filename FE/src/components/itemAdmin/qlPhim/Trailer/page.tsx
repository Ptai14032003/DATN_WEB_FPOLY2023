...
const TrailerPhim: React.FC<Props> = ({ data }: Props) => {
    ...
    const iframe = document.getElementById('video') as HTMLVideoElement;
    const showModal = () => {
        setIsModalOpen(true);
        setTrailer(data)
    };
    ...
    return (
        <>
            ...
            <Modal title="Trailer Phim" open={isModalOpen} onCancel={handleCancel} okButtonProps={{ hidden: true }} cancelButtonProps={{ hidden: true }} className="text-center">
                <iframe id="video" width="100%" height="400px" src={dataTrailer} title="YouTube video player" allow="accelerometer; ; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-sautoplayhare" allowFullScreen></iframe>
            </Modal>
        </>
    )
};
